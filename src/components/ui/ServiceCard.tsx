  ...
  return (
    ...
    >
      ...
      <div className="relative w-full aspect-[4/3] overflow-hidden bg-neutral-900">
        {service.image && !imgError ? (
          ...
          <div className="absolute inset-0 bg-gradient-to-br from-neutral-700 via-neutral-800 to-neutral-900 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-xs text-neutral-500 capitalize">{service.category.replace(/-/g, ' ')}</p>
            </div>
          </div>
        )}
        ...
        {/* Badge Top service */}
        {service.sponsored && (
          <div className="absolute top-3 left-3 glass text-primary-400 text-[10px] font-semibold px-3 py-1.5 rounded-full flex items-center gap-1.5">
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="flex items-center gap-3 mb-3">
          <div className="relative">
            <div className="w-8 h-8 rounded-full overflow-hidden bg-neutral-700 flex-shrink-0 ring-2 ring-primary-400/20">
              {freelance?.avatar ? (
                ...
                <div className="w-full h-full bg-primary-700 flex items-center justify-center text-xs font-bold text-black">
                  {freelance?.name?.[0] || '?'}
                </div>
              )}
            </div>
            {freelance?.verified && (
              <div className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 bg-primary-400 rounded-full border-2 border-neutral-900 flex items-center justify-center">
                <span className="text-[6px] text-black">✓</span>
              </div>
            )}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-xs font-medium text-neutral-300 truncate">{freelance?.name}</p>
            <p className="text-[10px] text-neutral-500 truncate">{freelance?.title}</p>
          </div>
        </div>

        {/* Titre */}
        <h3 className="text-sm text-neutral-200 line-clamp-2 mb-4 group-hover:text-white transition-colors leading-relaxed flex-1">
          {service.title}
        </h3>
        ...
        <div className="flex items-center gap-2 mb-4">
          <div className="flex items-center gap-0.5">
            {Array.from({ length: 5 }).map((_, i) => (
              <Star key={i} className={`w-3.5 h-3.5 ${i < Math.round(service.rating) ? 'fill-primary-400 text-primary-400' : 'text-neutral-700'}`} />
            ))}
          </div>
        ...
        {service.tags && service.tags.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-4">
            {service.tags.slice(0, 3).map((tag) => (
              <span key={tag} className="text-[10px] text-neutral-400 bg-neutral-800/50 px-2 py-1 rounded-full">
                {tag}
              </span>
            ))}
          </div>
